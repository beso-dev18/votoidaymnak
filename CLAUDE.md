...
│   ├── VD1 - Ham da (ngang gap 3).html/.pdf/.png  ← ⭐ bản đang dùng: A4 ngang in 1 mặt, gấp chữ Z thành 3 tấm 99mm; tấm 1 là bìa: gợi ý bộ SP (cần thiết / khuyến nghị) → sản phẩm → vấn đề được giải quyết → khi nào khuyên đi khám; tấm 2 = bước 1+2, tấm 3 = bước 3 + cách dùng theo tình trạng; mỗi bước có ô "Điểm mạnh khi khách so sánh" + câu nói với khách. Tờ 2 "Bán tại quầy": câu hỏi khai thác, 1 câu/SP, chốt bộ + giá cả bộ, 4 câu từ chối riêng hăm da, dặn mẹ (nhãn CHỜ XÁC NHẬN = chỗ chờ công ty chốt). PDF trong repo vẫn là bản cũ tới khi Nhi duyệt
│   └── VD1 - Ham da (3 cot).html/.pdf/.png  ← poster A4 3 cột chuyển từ mục 1 "Hăm da" của Sell-out kit theo vấn đề da bé (bản chạy thử để duyệt mẫu; 8 mục còn lại chưa dựng). 3 cột = 3 bước, ảnh SP (web công ty), ảnh dược liệu lấy từ các slide pptx của công ty (thiếu thì lấy Wikimedia Commons), ảnh 5 cấp độ hăm của công ty; nguồn từng ảnh ghi trong README
└── Sellout kit/                        ← chuyển từ Phân tích công việc/7. Hỗ trợ điểm bán/ sang, gom mọi tài liệu ngoài "Phân tích công việc.md" và "Câu hỏi làm rõ.md"
    ├── Khung xử lý từ chối mua hàng - khối vấn đề (toàn bộ SP OTC).md  ← framework tầng cao hơn dành cho NV shop tư vấn NTD tại điểm bán: nhóm câu từ chối thành 6 khối vấn đề chung cho toàn bộ 13 SP/dòng trong bảng báo giá OTC (thương hiệu, giá, thành phần, hiệu quả, pháp lý, trì hoãn quyết định) + phương hướng trả lời chung và câu mở dẫn dắt theo công thức sandwich 3 lớp (Đồng cảm → Làm rõ + lợi thế → Điều hướng) cho mỗi khối, không phải câu trả lời cụ thể từng câu hỏi. Không bao gồm khối vấn đề riêng của đối tác kênh (NPP/Shop/CTV cân nhắc nhập hàng)
    ├── Bảng khối vấn đề từ chối mua hàng (bản đầy đủ).docx  ← bảng Word Nhi tự soạn/chỉnh (A4 ngang, ~5 trang): 9 khối vấn đề × 6 cột (khối vấn đề, câu hỏi thường gặp, insight ngầm, phương hướng trả lời, câu mở sandwich 3 lớp) — thêm 3 khối so với file .md: đang dùng loại khác, sợ dị ứng, theo lời khuyên người quen
    ...
    │   ├── Sell-out kit theo vấn đề da bé.docx  ← ⭐ BẢN CHÍNH (Word A4 ngang): mỗi vấn đề một mục lớn + bảng gộp ô 6 cột (vấn đề da → bước → sản phẩm → thành phần → giúp gì cho bé, viết lời thường, không thuật ngữ → điểm mạnh khi khách so sánh + câu nói với khách). Bố cục theo bản Nhi sửa (không có phần mở đầu/phần cuối)
    ...
    │   └── So do - 1..9 - <tên>.pdf    ← sơ đồ nhánh riêng từng vấn đề, in lẻ khi cần; cột so sánh chỉ giữ dòng "Điểm mạnh"
...
├── Mẫu DKX (cá nhân) - ... tuần 21-26.09.2026.docx  ← báo cáo tuần 21–26/09 theo bản cá nhân; các ô [Điền] là số liệu Nhi tự bổ sung (AMIS, doanh số, shop, ngày công)
